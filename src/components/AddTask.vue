<template>
    <div class="flex flex-col items-center justify-center px-6 py-8 my-4 lg:py-0">
...
export default {
    ...
    methods: {
        addTask() {
            // Call the Vuex action to add the task
            this.$store.dispatch('addTask', {
                title: this.title,
                description: this.description
            }).then(() => {
                // Optionally, handle success (e.g., navigate to task list or clear form)
                this.title = '';
                this.description = '';
            }).catch(error => {
